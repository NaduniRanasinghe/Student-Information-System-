...
const Todo = require('./models/lecturer');
...
todoRoutes.route('/add').post(function(req, res) {
    ...
    todo.save()
        .then(todo => {
            res.status(200).json({'todo': 'lecturer added successfully'});
        })
        .catch(err => {
            res.status(400).send('adding new lecturer failed');
        });
});
...
app.use('/lecture', todoRoutes);
//end of the addings by pasan
